...
namespace Services
{
    public class PaymentService : IPayment
    {
        ...
        {
            return _context.Payments.Include(p => p.Customer).OrderByDescending(p => p.Date);
        }


        public IEnumerable<Payment> GetAllFromCustomer(int customerId)
        {
            return GetAll().Where(p => p.Customer.Id == customerId);
        }


        public double GetPaymentSum(int customerId)
        {
            double sum = 0;
